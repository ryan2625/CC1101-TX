 ...
 * Author: Ryan Freas
 * Date: 3/15/2026
 * Project: ESP32-CC1101-Transmission
 *
 * Description:
...
// ================= STROBES ================ //  // Strobe Description
constexpr uint8_t CC1101_STROBE_SRES      = 0x30; // Reset Chip
...
constexpr uint8_t CC1101_CONFIG_PATABLE   = 0x3E; // PATABLE[0]
constexpr uint8_t CC1101_CONFIG_SYNC1     = 0x04; // SYNC[15:8]
constexpr uint8_t CC1101_CONFIG_MDMCFG1   = 0x13; // NUM_PREAMBLE
constexpr uint8_t CC1101_CONFIG_PKTCTRL0  = 0x08; // LENGTH_CONFIG
constexpr uint8_t CC1101_CONFIG_PKTLEN    = 0x06; // PACKET_LENGTH
...
// ============= STATUS REGISTERS ============ //
constexpr uint8_t CC1101_STATUS_TXBYTES   = 0x3A; // NUM_TXBYTES
constexpr uint8_t CC1101_STATUS_MARCSTATE = 0x35; // MARC_STATE[4:0]
// =================== FIFO ================== //
constexpr uint8_t CC1101_REG_FIFO         = 0x3F; // TX FIFO
// ================== VALUES ================= //
...
constexpr uint8_t CC1101_DUMMY_BYTE       = 0x00; // Byte Used for Reading Data (Full-Duplex Dummy Byte)
// =========================================== //
...
uint8_t calculate_header_byte(uint8_t address, bool read, bool burst) {
    ...
    values below to get the resulting header byte. See Assets/bit_shift.png for more info.
...
void spi_transaction(spi_device_handle_t cc1101, const uint8_t* data, size_t len,  const std::string& operation, bool shouldLog = false) {
    spi_transaction_t t = {};
    uint8_t rx[len];
    t.tx_buffer = data; // tx[0] will always be the header byte
