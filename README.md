...
The CC1101 recommends implementing a 4-byte sync word, stored in the `SYNC1` and `SYNC0` registers. Their addresses are `0x04` and `0x05` respectively. To reach the recommended 4 bytes in a sync word, we will have to send the `MDMCFG2.SYNC_MODE` field either a 3 (`011`) or a 7 (`111`). This will duplicate the 2 bytes we have stored in `SYNC1` and `SYNC0` and send 4 bytes in total when we transmit data. 
